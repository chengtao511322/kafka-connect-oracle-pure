...
public class OracleSourceConnectorUtils{
    ...
    protected String getDbTime() throws SQLException {
        Date date = null;
        PreparedStatement dbTimePs = dbConn.prepareCall("select sysdate from dual");
        ResultSet dbTimeRs = dbTimePs.executeQuery();
        while (dbTimeRs.next()){
            date = dbTimeRs.getDate(1);
        }
